...
class JwtAuthenticationTokenService(private val jwtKey: String) : AuthenticationTokenService {
    ...
    override fun readAccessToken(tokenStr: String): AuthenticationToken? {
        ...
        try {
            ...
            throw WindySecurityException("access tokenStr 过期!")
        }
        if (claims != null) {
            return try {
                val tokenMap = claims.body["tokenStr"]
                if (tokenMap == null) {
                    null
                } else {
                ...
                e.printStackTrace()
                throw WindySecurityException(e.message ?: "tokenStr 解析错误!")
            }
        } else {
    ...
    override fun createAccessToken(authenticationToken: AuthenticationToken): String {
        ...
        val expireTime = date + authenticationToken.expireIn!! * 1000

        val builder = Jwts.builder()
//                .setId()
                .setIssuedAt(Date(date))
                .claim("token", authenticationToken)
                .setSubject("access_token ${authenticationToken.authentication!!.principal}")
